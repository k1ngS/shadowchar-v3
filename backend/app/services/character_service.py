...
class CharacterService:
    ...
    def create_character(
        db: Session, character: CharacterCreate, user_id: uuid.UUID
    ) -> Character:
        """Criar novo personagem"""
        db_character = Character(user_id=user_id, **character.dict())
        db.add(db_character)
        db.commit()
    ...
    def get_characters_by_user(
        ...
        return (
            db.query(Character)
            .filter(Character.user_id == user_id, Character.is_active)
            .offset(skip)
            .limit(limit)
            .all()
        )
    ...
    def get_character_by_id(
        db: Session, character_id: uuid.UUID, user_id: uuid.UUID
    ) -> Optional[Character]:
    ...
    def get_character_stats(db: Session, user_id: uuid.UUID) -> dict:
        ...
        avg_level = (
            db.query(Character)
            .filter(Character.user_id == user_id, Character.is_active)
            .with_entities(Character.level)
            .all()
        )

        avg_level_value = (
            sum([char.level for char in avg_level]) / len(avg_level) if avg_level else 0
        )

        return {"total_characters": total, "average_level": round(avg_level_value, 1)}
